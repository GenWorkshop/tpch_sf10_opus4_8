...
private:
    void load_fresh() {
        close();
        // Drop the previous hot-reload copy now that its handle is closed,
        // so the .reload/ directory does not grow without bound across the
        // many rebuilds of a long-running synthesis session.

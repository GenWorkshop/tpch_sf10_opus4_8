...
// Ask the kernel to deliver SIGKILL to the calling process the instant its
// parent dies for any reason — including the OOM killer or a hard crash that
// no userspace cleanup path can catch. PDEATHSIG is cleared across fork(), so
// every forked pipeline stage must re-arm it for itself; otherwise a stage
// stuck in a busy loop (e.g. a non-terminating synthesized query) would be
// reparented to init and keep burning CPU and holding gigabytes of mapped
// parquet / in-memory database long after the engine is gone.
static inline void install_parent_death_signal() {
    ::prctl(PR_SET_PDEATHSIG, SIGKILL);
    // Close the race where the parent died between fork() and the prctl call:
    // if we have already been reparented to init, exit now.
    if (::getppid() == 1) {
        _exit(0);
    }
}
...
struct StageDef {
    ...
    const char* so_path;
    Compute compute;
    // Optional hook invoked with the still-loaded (old) plugin immediately
    // before a hot-reload, so the stage can free any heap state produced by
    // the previous plugin instead of leaking it inside the long-lived process.
    void (*release)(Plugin&) = nullptr;
};
...
static StageDef<P, detail::compute_input_t<Compute>, Compute> make_stage(
    ...
    Compute compute,
    void (*release)(Plugin&) = nullptr) {
    return StageDef<P, detail::compute_input_t<Compute>, Compute>{so_path, compute, release};
}
...
template <RunPolicy P, class Compute>
static auto stage(const char* so_path, Compute compute, void (*release)(Plugin&) = nullptr) {
    return detail::make_stage<P>(so_path, compute, release);
}
...
static void stage_loop_impl(
    ...
    auto do_run = [&]() {
        ...
        if (reload) {
            ...
            // Release heap state built by the outgoing plugin while it is still
            // loaded; doing it after reload() would run the new plugin's
            // destructor over an object laid out by the old one.
            if (has_run && release) {
                release(plugin);
            }
            plugin.reload();
